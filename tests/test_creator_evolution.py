...
class CreatorEvolutionTests(unittest.TestCase):
    ...
    def test_has_separate_state_and_route_contract(self):
        ...
        self.assertIn("def _x_oauth2_callback_requested", app_text)
        self.assertIn('marker == "callback"', app_text)
        self.assertIn('st.query_params.get("code"', app_text)
        self.assertIn('st.query_params.get("state"', app_text)
        ...
        self.assertIn("def _x_direct_post_readiness", app_text)
        self.assertIn("def _render_x_direct_post_setup", app_text)
        self.assertIn("oauth2_reconnect_required", app_text)
        self.assertIn("oauth2_expires_at > time.time() + 120 or oauth2_has_refresh", app_text)
        self.assertIn("Direct posting needs a secure X connection", app_text)
        self.assertIn("No tweet was sent.", app_text)
        self.assertIn("Manual X Composer Fallback", app_text)
        self.assertIn('if not _x_direct_post_readiness().get("ready")', app_text)
    ...
    def test_live_sports_context_prioritizes_avalanche_games_and_news(self):
        ...
        self.assertIn("_signal_choices = _ce_pulse_choice_candidates(_decision, lane=_lane, fmt=_fmt, minimum=3)[:3]", pulse_dialog)
        self.assertIn('st.selectbox(\n            "Choose Pulse Signal"', pulse_dialog)
        self.assertIn("Emergency Pulse backup", app_text)
        self.assertIn('[class~="st-key-ce_pulse"]', app_text)
        self.assertNotIn('[class*="st-key-ce_pulse"]', app_text)
    ...
    def test_creator_evolution_hot_feed_uses_source_backed_cards_before_fallback(self):
        ...
        self.assertIn("_min_hot_cards = 5", hot_block)
        self.assertIn("_raw_ideas = _raw_ideas + _build_inspiration_fallback(_all_tweets, _rss_headlines)", hot_block)
        self.assertIn("for _card in (_pulse_cards or [])", hot_block)
        self.assertIn("if len(_ideas) < _min_hot_cards", hot_block)
        self.assertIn("for _starter in _creator_evolution_starter_hot_ideas()", hot_block)
        self.assertIn("_ideas = _ideas[:_min_hot_cards]", hot_block)
        self.assertIn("_ce_hot_all_ten(_card)", hot_block)
        self.assertIn('if str((idea or {}).get("source") or "").strip().lower() != "starter fallback"', hot_block)
        self.assertIn("_ce_hot_signal_card_from_idea(_starter, _lane, _fmt)", hot_block)
        ...
        self.assertIn("Details", ui_block)
        self.assertIn("_per_page = 5", ui_block)
    ...
    def test_app_pulse_dialog_has_fail_closed_boundary(self):
        ...
        self.assertIn("html.unescape", app_text)
        self.assertIn('"special promotion"', app_text)
        self.assertIn('"gift card"', app_text)
        self.assertIn('"sportspredict"', app_text)
        self.assertIn("visible_card_text", app_text)
        self.assertNotIn('st.query_params.get("open_pulse")', app_text)
        self.assertNotIn('st.query_params.get("pulse_signal")', app_text)
